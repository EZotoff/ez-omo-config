...
7. **Stop servers (both surfaces)**: `systemctl --user stop opencode.service opencode-interactive.service`. Both units carry continuation hooks — ExecStop snapshots busy sessions automatically, and the next start resumes them (TTL 1h). If a non-systemd `opencode serve` process is still running, inspect `pgrep -af 'opencode serve'` and stop only the specific service-owned PID that is holding the live binary. Do not run broad `pkill`/`kill -9` loops; if more processes match than expected, stop and choose manually. Otherwise the swap can fail with `Text file busy` or kill unrelated sessions. **Restart BOTH services after the swap** — stopping both and restarting only one leaves the other down (2026-09-17 10:35 incident: opencode.service down ~3h).
8. **Install the patched binary**: `cp dist/opencode-linux-x64/bin/opencode ~/.opencode/bin/opencode && chmod +x ~/.opencode/bin/opencode`
9. **Restart servers**: `systemctl --user start opencode.service opencode-interactive.service` — ExecStartPost continuation hooks resume any sessions that were busy at the stop.
