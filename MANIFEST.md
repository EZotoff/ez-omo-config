...
| 4ab | agent/prometheus-planner.md | `~/.config/opencode/agent/` | `configs/opencode/agent/` | `$HOME/.config/opencode/agent/` | Opencode-native agent file for custom `prometheus-planner` subagent (worker-class Prometheus planning clone: K3 high, ulw-plan doctrine, QUESTIONS/ANSWERS escalation protocol; coordinator `prometheus` itself stays unspawnable per OMO guard) | Required |
...
| 11j | agent-git-workflow.ts | `~/.opencode/plugin/` | `plugins/` | `$HOME/.opencode/plugin/` | Agent Git Workflow (commit reflex + branch lifecycle guidance plugin) | Required |
...
| 54b | `integrity-triage.sh` | `~/.sisyphus/scripts/` | `scripts/` | `$HOME/.sisyphus/scripts/` | Triage gate between integrity-check failure and operator notification: deterministic auto-remediation (smoke matrix for pending provenance, stale-marker cleanup), headless agent for judgment-call failures (live-config drift commit/hold, fork-remote push), fingerprint-deduped escalation (12h window, 2 agent runs/day budget) | Optional |
...
| 62b-h | `opencode.service.d/continuation.conf` | `~/.config/systemd/user/opencode.service.d/` | `systemd/user/opencode.service.d/` | `$HOME/.config/systemd/user/opencode.service.d/` | Continuation drop-in for the headless :3021 server (same ExecStop/ExecStartPost mechanism, serve.env auth). Note: the parent `opencode.service` unit is machine-local and not repo-tracked | Optional |
...
| 62h | `opencode-integrity-triage.service` | `~/.config/systemd/user/` | `systemd/user/` | `$HOME/.config/systemd/user/` | OnFailure triage unit — replaces the direct alert path as OnFailure target of `opencode-patch-integrity-check.service`; auto-remediates, escalates to `opencode-integrity-alert.service` only for unresolved, undeduplicated failures | Optional |
...
| 63c | `check-remote-presence.sh` | (repo only) | `scripts/` | (repo only) | Remote-presence drift check: parses "fork commit(s) <hashes>" citations from active patch entries, queries the GitHub API for each hash on `EZotoff/oh-my-openagent`, and fires a drift alert (marker `~/.local/state/opencode/patch-remote-drift.alert` + notify) when any cited commit is missing from the remote. Self-throttled to daily; third `ExecStart` of `opencode-patch-integrity-check.service`. Created after the 2026-09-18 incident where the quota-only entry cited five never-pushed commits | Required |
| 63f | `config/patch-lockfile.json` | (repo only) | `config/` | (repo only) | Opencode patch-set lockfile (generation `opencode-1.18.5-patches.1`): 9 active patches with full implementation-commit SHAs, canonical ref `fork/fix/all-patches-v1.18.5`; machine-authoritative provenance source (sse-directory-filter-removal = acknowledged-drift, exempt) | Required |
| 63g | `config/omo-lockfile.json` | (repo only) | `config/` | (repo only) | OMO patch-set lockfile (generation `omo-4.19.2-patches.1`): 8 source patches + 5 dist patches, canonical ref `origin/fix/custom-patches-v4.19.2` | Required |
| 63h | `test_patch_lockfile.sh` | (repo only) | `tests/` | (repo only) | Lockfile validator: JSON parse, entry bijection, full-SHA enforcement, git ancestry vs canonical ref, fork-remote presence | Required |
| 63i | `build-and-install-opencode.sh` | (repo only) | `scripts/` | (repo only) | Transactional opencode installer: lockfile ancestry gate → bun build (`-p<gen>` version suffix) → receipt (`~/.local/share/opencode/builds/<sha>.json`) → gated swap with backup; `--recovery-from` mode writes persistent red marker; `verify --bootstrap` one-time blesses the current binary | Required |
| 63j | `build-and-install-omo.sh` | (repo only) | `scripts/` | (repo only) | OMO dist equivalent: receipt gate for `~/oh-my-openagent-v4.19.2/dist/index.js` (`omo-<sha>.json` receipts), dist verification_pattern recheck, `--bootstrap` support | Required |
...
|---|----------|------|---------|-----------------|
| E1 | `auth.json.example` | `auth.json.example` | Template for `~/.local/share/opencode/auth.json`: 9 provider entries (7 API-key + 2 OAuth) covering the enabled providers (parked provider blocks reuse the same keys) | `cp auth.json.example ~/.local/share/opencode/auth.json` |
...
| E8 | `voice-bridge` (Vox) | `~/AI_projects/voice-bridge/` (external project) | Phone push-to-talk voice agent over Gemini Live that supervises the OpenCode fleet via the supervisor ledger; sibling Bun service on `127.0.0.1:18220`. Voice layer of the Portable Supervisor prototype (contract: `docs/portable-supervisor-contract.md`). Evidence: `live_file_installed` + `runtime_loaded` (service active, 118 tests, text-loopback e2e); `real_project_behavior_proven` pending real-voice dogfood | `cd ~/AI_projects/voice-bridge && ./deploy/install.sh` |
...
| 4 | `omo--boulder-worktree-authoritative-state` | `oh-my-openagent` | superseded | 2026-05-19 | `grep -n "worktreePath\|effectiveDirectory\|displayDirectory" ~/omo-hub/projects/oh-my-openagent/src/hooks/atlas/resolve-active-boulder-session.ts` |
...
| 11 | `omo--remove-activity-stagnation-bypass` | `oh-my-openagent` | upstreamed | 2026-04-10 | `grep -n '"none" \| "todo"' ~/omo-hub/projects/oh-my-openagent/src/hooks/todo-continuation-enforcer/session-state.ts` |
...
| 34 | `omo--family-reasoning-efforts` | `oh-my-openagent@4.19.2` | active | 2026-09-15 | `grep -c 'reasoningEfforts: ["low", "medium", "high", "max"]' ~/oh-my-openagent-v4.19.2/dist/index.js` — per-family reasoning-effort ladders (glm/deepseek/kimi); source port restored 2026-09-18 as fork commit ba8fc048e after the original port was lost with the runtime dir (never pushed); dist + source both verified applied |
