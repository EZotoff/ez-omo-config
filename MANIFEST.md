...
| 10b | auto-checkpoint.ts | `~/.opencode/plugin/` | `plugins/` | `$HOME/.opencode/plugin/` | Auto-Checkpoint | Required |
...
│   ├── auto-checkpoint.ts  # Auto-checkpoint semantic scaffolding plugin
...
- **Plugins**: 8 main files + vscode.ts + session-id.ts + session-info.ts + vera-runtime.ts + auto-checkpoint.ts + worktree/ (2 files) + kdco-primitives/ directory
