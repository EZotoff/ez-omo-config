...
- **OpenCode TUI pinned-session reset**: `.sisyphus/patches/opencode--tui-pinned-session-race.md` (FIXED locally 2026-08-15, source commit e7f5981ea, compiled into live v1.18.5 binary; `runtime_effective: false` until live pin+prune verification — startup read-overwrite race + stale multi-process prune overwrite on `~/.local/state/opencode/session.json`; regression pair `tests/regressions/012-pinned-session-race-fix.sh`)
