...
|---|---|---|
| `AGENTS.md` | Global user-level agent instructions loaded by OpenCode on top of any project-level `AGENTS.md`. Currently mandates the `/deployment` skill before binding ports or launching dev/test servers, mandates session-safe server restarts via `scripts/restart-with-continuation.sh` (no bare `systemctl` restarts; snapshot + auto-continue active sessions), uses vanilla code discovery guidance, and mandates plan-execution records: every executed plan appends `Execution baseline:` to its `## Execution Record` before dispatch and `F#<n>: <verdict>` lines at closeout, with `verify-built` flagging missing records as GAPs. Atomic-install tag: `skills+configs`. | `$HOME/.config/opencode/AGENTS.md` |
