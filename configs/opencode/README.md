...
| `session-info.ts` | Intercepts `/session-info` and copies `Project <path>:<branch>; Session <title>; ID <session-id>` to clipboard via `command.execute.before`. Uses `output.parts` in-place mutation to suppress the LLM round-trip (non-throw abort — OpenCode 1.17.5+ surfaces throws as TUI error toasts, upstream issue #32253). Evidence state: `repo_implemented`, `active_config_registered`; not verified live: `runtime_loaded`, `real_project_behavior_proven`. | `$HOME/.opencode/plugin/session-info.ts` |
| `session-id.ts` | Intercepts `/session-id` and copies the invoking session ID to clipboard. Same non-throw abort pattern as `session-info.ts`. Evidence state: `repo_implemented`, `active_config_registered`; not verified live: `runtime_loaded`, `real_project_behavior_proven`. | `$HOME/.opencode/plugin/session-id.ts` |
| `vscode.ts` | Intercepts `/vscode` and launches VS Code in the current directory. Same non-throw abort pattern. Evidence state: `repo_implemented`, `active_config_registered`; not verified live: `runtime_loaded`, `real_project_behavior_proven`. | `$HOME/.opencode/plugin/vscode.ts` |
