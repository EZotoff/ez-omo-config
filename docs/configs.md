...
**Doom-Loop Mitigations** (added 21 Jun 2025): Four layered defenses against runaway subagent sessions — (1) `experimental.preemptive_compaction: true` triggers compaction before context exhaustion; (2) `opencode.json#compaction.reserved: 30000` reserves 30k tokens of headroom so compaction fires earlier; (3) `experimental.dynamic_context_pruning.strategies.purge_errors.turns: 2` drops failed tool outputs after 2 turns instead of 5; (4) `babysitting.timeout_ms: 600000` plus `categories.visual-engineering.is_unstable_agent: true` enables 10-minute idle reminders for the worst-offender category. These are all global/subagent-scoped; OMO schema has no per-category `compaction` override or per-session tool-call budget.
