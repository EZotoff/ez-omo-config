...
**Notable Model Assignments**: GPT-family routes use the Codex/OpenAI provider (`openai/*`). `provider.openai.whitelist` limits the model picker to `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; OpenCode's built-in provider catalog would otherwise add older GPT models and fast/pro variants. `agents.multimodal-looker.model` uses `openai/gpt-5.6-terra` for image/PDF analysis. The `visual-engineering` category uses `google/gemini-3.7-flash` (high thinking) for frontend work — demoted from `gemini-3.1-pro-preview` on 21 Jun 2025 to cap per-session cost after forensic analysis of two runaway subagent sessions; upgraded from 3.5 Flash to 3.6 Flash on 21 Jul 2026 following GA release, and to 3.7 Flash on 15 Aug 2026 at release (most capable Flash model; intro pricing $0.75/$3.75 per 1M in/out tokens). The `frontend-ui-ux-engineer` agent uses `zai-coding-plan/glm-5.3` (max thinking) with `openai/gpt-5.6-sol` fallback. The `artistry` category uses `google/gemini-3.7-flash` (high thinking) as its primary (switched from `gemini-3.1-pro-preview` on 15 Aug 2026). `momus` uses `openai/gpt-5.6-sol` (xhigh) as primary and `metis` uses `zai-coding-plan/glm-5.3` (max thinking) as primary, both with `gemini-3.1-pro-preview` as a strong secondary fallback; `oracle` uses `openai/gpt-5.6-sol` as primary with a three-model fallback chain of `kimi-for-coding-oauth/kimi-for-coding` → `zai-coding-plan/glm-5.3` → `google/gemini-3.1-pro-preview` (reordered 22 Jul 2026 to put K3 first for stronger reasoning fallback). Both Google models are defined in `opencode.json` under `provider.google.models`. OpenCode Go-backed discovery agents use `opencode-go/minimax-m3`, the `writing` category and `document-writer`/`mephistopheles` use `openai/gpt-5.6-terra` as primary (switched from Sol 15 Aug 2026; `mephistopheles` carries `opencode-go/deepseek-v4-pro` as first fallback ahead of `zai-coding-plan/glm-5.3` and Kimi), and every chain containing `openai/gpt-5.6-sol` carries `opencode-go/deepseek-v4-pro` as the fallback immediately behind it (added 15 Aug 2026 for cross-provider Sol relief). K3 (`kimi-for-coding-oauth/kimi-for-coding`) is reserved for high-leverage roles only: `prometheus` (primary), `oracle` (first fallback), and `ultrabrain` (first fallback; primary is `zai-coding-plan/glm-5.3` max since 15 Aug 2026); `atlas` retains K3 as a deep fallback (after Sol). The model id MUST stay `kimi-for-coding` (not `k3` or any other alias) — `opencode-kimi-full` plugin's `resolveKimiBodyFields` hard-gates on it, and bypassing it disables `thinking:{type:"enabled"}` injection, `prompt_cache_key`, wire-slug discovery, and effort clamping. Bug introduced 2026-07-21 in commit 401318c, fixed 2026-08-07.
