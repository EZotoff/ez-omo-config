...
**Notable Model Assignments**: GPT-family routes use the Codex/OpenAI provider (`openai/*`). `provider.openai.whitelist` limits the model picker to `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; OpenCode's built-in provider catalog would otherwise add older GPT models and fast/pro variants. `agents.multimodal-looker.model` uses `openai/gpt-5.6-terra` for image/PDF analysis. The `visual-engineering` category uses `google/gemini-3.6-flash` (high thinking) for frontend work — demoted from `gemini-3.1-pro-preview` on 21 Jun 2025 to cap per-session cost after forensic analysis of two runaway subagent sessions; upgraded from 3.5 Flash to 3.6 Flash on 21 Jul 2026 following GA release (improved token efficiency and code/agentic planning at a lower price point). The `frontend-ui-ux-engineer` agent uses `zai-coding-plan/glm-5.2` (high thinking) with `openai/gpt-5.6-sol` fallback. The `artistry` category still uses `google/gemini-3.1-pro-preview` as its primary. Specialist agents (`metis`, `momus`) use `openai/gpt-5.6-sol` as primary with `gemini-3.1-pro-preview` as a strong secondary fallback; `oracle` uses `openai/gpt-5.6-sol` as primary with a three-model fallback chain of `kimi-for-coding-oauth/k3` → `zai-coding-plan/glm-5.2` → `google/gemini-3.1-pro-preview` (reordered 22 Jul 2026 to put K3 first for stronger reasoning fallback). Both Google models are defined in `opencode.json` under `provider.google.models`. OpenCode Go-backed discovery agents use `opencode-go/minimax-m3`, and the `writing` category uses `openai/gpt-5.6-sol` as its primary (with `zai-coding-plan/glm-5.2` as fallback). K3 (`kimi-for-coding-oauth/k3`) is reserved for high-leverage roles only: `prometheus` (primary), `ultrabrain` category (primary), and `oracle` (first fallback); `atlas` retains K3 as a deep fallback (after Sol).
