...
- Watcher: `scripts/watch-runtime-patches.sh` (`opencode-patch-watcher.service`) — inotify (`close_write,moved_to,moved_from,create,delete`) on `~/.opencode/bin/` (files matching `opencode$`) and `$HOME/oh-my-openagent-v4.19.2/dist` (recursive), plus a NON-recursive parent guard on `$HOME` that alerts (integrity-alert path) when the runtime dir `~/oh-my-openagent-v4.19.2` itself is deleted/moved (2026-09-18 incident). The watcher re-arms in a loop if a watched tree is deleted. dist writes are expected ONLY during verified rebuild+reapply flows (patch-tracker/update-to-latest); any other dist write is suspect. `*.pre-*` / `*.tmp*` dist writes are legitimate patch-flow backups and are logged informationally only — never alerted.
- Remote-presence drift check: `scripts/check-remote-presence.sh` (third `ExecStart` of `opencode-patch-integrity-check.service`, self-throttled to daily). Parses "fork commit(s) <hashes>" citations from active patch entries and asks the GitHub API whether each hash exists on `EZotoff/oh-my-openagent`. On drift: marker `~/.local/state/opencode/patch-remote-drift.alert` + critical `notify-send` + journal line. Created after the 2026-09-18 incident where an active entry cited five fork commits that had never been pushed; see AGENTS.md Cooperation Contract item 8.
- Triage-before-alert: on check failure, `OnFailure=` now fires `opencode-integrity-triage.service` (`scripts/integrity-triage.sh`) instead of alerting directly. Triage auto-runs pending smokes, dispatches a headless agent for judgment-call failures (live-config drift commit-vs-hold, fork-remote push), and escalates to `opencode-integrity-alert.service` only when the failure survives remediation AND the same failure fingerprint was not escalated within 12h — same-failure repeat cycles are journal lines only. State: `~/.local/state/opencode/integrity-triage.json`.
- Failure alerting (escalation path, invoked by triage): `opencode-integrity-alert.service` writes a marker (`~/.local/state/opencode/patch-integrity.alert`), logs `INTEGRITY-ALERT: patch verification failing` to the journal, and sends a critical `notify-send` (guarded for headless rigs). Bridges the 2026-09-08 4+-hour silent-failure gap.
