...
**Runtime default**: Enabled on this machine via `OPENCODE_AUTO_CHECKPOINT_ENABLE=1` set in **both** EnvironmentFiles that feed OpenCode servers: `~/.config/opencode/serve.env` (for `opencode.service` on port 3021) AND `~/AI_projects/omo-tg/.env` (for `omo-tg.service`, which spawns its own `opencode serve --port 4096` child for Telegram-driven sessions). File logging is also enabled via `OPENCODE_AUTO_CHECKPOINT_FILE_LOG=1` in both files. On a fresh install without these env vars, the plugin loads but returns empty hooks (no checkpoints). To enable on another machine, set both env vars in whatever EnvironmentFile(s) the OpenCode systemd units consume, then restart the corresponding services.
