...
- `review-enforcer.ts` — injects review workflow instructions after task completion so plan execution gets reviewed consistently. Injection is gated: consultative subagent dispatches (oracle/metis/momus/explore/librarian/multimodal-looker/document-writer) are skipped — analysis work has no implementation to review — and the plan-complete instruction fires only for sessions inside the active boulder's `session_ids` lineage (mirrors OMO's `resolveActiveBoulderSession` predicate, `opencode:` prefix normalized) when the boulder status is not paused/abandoned. Guards against a stale machine-global `boulder.json` hijacking unrelated sessions (2026-08-16 debate-session misfire; regression pairs `tests/regressions/014-review-enforcer-plan-lineage` and `015-review-enforcer-consultative-denylist`).
