...
- `auto-checkpoint.ts` — semantic session-scoped git checkpointing. Enabled on this machine via `OPENCODE_AUTO_CHECKPOINT_ENABLE=1` in `~/.config/openchamber/openchamber.env`. Timings: 30s idle threshold, 5min cooldown. On a fresh install without the env var, plugin loads but returns empty hooks.
