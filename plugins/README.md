...
- `agent-git-workflow.ts` — force-loads (via `experimental.chat.system.transform`) a parallel-agent git coordination procedure into every session's system prompt: commit reflex, machine-parseable commit format with per-commit author override and `Session:` trailer, branching reflex when concurrent agent work is detected, sync/rebase protocol, branch lifecycle. Complements auto-checkpoint (which handles idle-timeout safety-net commits) and the commit-policy patches (which grant permission).
