...
- `session-id.ts` — copies the invoking session ID to clipboard via `/session-id`. Uses `output.parts` in-place mutation to suppress the LLM round-trip (non-throw abort since OpenCode 1.17.5+, upstream issue #32253).
- `session-info.ts` — copies `Project <path>:<branch>; Session <title>; ID <session-id>` to clipboard via `/session-info` command. Same non-throw abort pattern as `session-id.ts`.
