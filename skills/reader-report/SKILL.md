...
- **Warm near-black paper background**, not pure black (`#1B1814` / `#211E18` family). Pure `#000` flattens warm accents and reads as "app terminal"; a tinted dark holds the "document" register.
- **Tinted near-white ink**, not pure `#FFF` (`#EDE9DF` / `#E2DDD0` family). Pure white on warm dark is glaring; the tint keeps the editorial register.
- **One restrained accent colour** for emphasis (amber `#D9A441`, teal `#4DAFB6`). Lifted from their light-theme values (`#C8901E` / `#006A71`) to hold contrast on dark. Used sparingly — for focus, links, key callouts. Never as decoration.
...
- **Callout boxes** for the doctrinal line, the key insight, the anchor scenario — visually distinct (a panel lifted or recessed relative to the paper, with a tinted left border in the accent colour). Use sparingly; a page of callouts is a page of nothing.
...
- **Low-contrast grey text on tinted ground.** Kills contrast in either theme; dark backgrounds are not a licence for murky mid-greys.
- **Pure black backgrounds / pure white ink.** Always tint — `#000`↔`#FFF` is the neutral, dated look on either theme.
