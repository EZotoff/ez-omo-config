#!/usr/bin/env bash
# Cleanup for bash-pipe-wedge-bounded.sh.
#
# Kills exactly the escaped (setsid) child the test recorded — the process the
# bash tool deliberately does not own. Idempotent: safe to run twice; exits 0
# when the PID list is empty or the process is already gone. No pkill sweeps —
# every kill is a specific recorded PID, guarded by a cmdline identity check so
