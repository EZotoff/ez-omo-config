...
scenario_event_lifecycle() {
    ...
    assert_contains "OMO_VERA_RUNTIME_AUTOSTART" "$plugin_src" "manual-by-default autostart flag missing"
    assert_contains "if (!AUTO_BOOTSTRAP_ENABLED)" "$plugin_src" "unconditional manual-mode startup guard missing"
    assert_contains "recordManualMode(directory, state)" "$plugin_src" "manual-mode startup guard missing"
    assert_contains "if (!AUTO_BOOTSTRAP_ENABLED) return" "$plugin_src" "manual-mode health-check guard missing"
    assert_contains "OMO_VERA_RUNTIME_TOOL_UPDATE" "$plugin_src" "tool update opt-in flag missing"
